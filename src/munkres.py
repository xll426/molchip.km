...
if __name__ == '__main__':
 ...
 [-1.0000, -1.0000,]],
    ...
    for cost_matrix, expected_total in matrices:
        # print("type(cost_matrix)",type(cost_matrix))

        # 对 cost_matrix 进行取反，跳过 DISALLOWED_OBJ 类型的对象
    
        cost_matrix = [[(0 if isinstance(element, DISALLOWED_OBJ) or (isinstance(element, (int, float)) and element < 0) else -element) for element in row] for row in cost_matrix]
        ...
        print(('highest cost=%s' % -total_cost))
